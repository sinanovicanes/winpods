...
struct OnAutoStartChangeEvent(pub bool);
...
impl SettingsState {
    pub fn set_auto_start(&mut self, new_state: bool) {
        if self.auto_start == new_state {
            return;
        }
        self.auto_start = new_state;
        self.dispatcher.dispatch(OnAutoStartChangeEvent(new_state));
    }
    ...
    pub fn on_auto_start_changed(&self, callback: impl Fn(&bool) + Send + Sync + 'static) {
        self.dispatcher
            .add_listener::<OnAutoStartChangeEvent, _>(move |event| {
                callback(&event.0);
            });
    }
...
pub fn init(app: &mut App) {
    ...
    settings_state.on_auto_start_changed(move |state| {
        app_handle
            .emit("settings:update:auto_start", state)
            .unwrap();
        let store = app_handle.store("settings.json").unwrap();
        store.set("auto_start", *state);
    });
